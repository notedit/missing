...
class LoginForm(Form):
    ...
    password = PasswordField(validators=[required(message='密码没有填写')])
    submit = SubmitField()

class SignupForm(Form):

    next = HiddenField()
    username = TextField(validators=[required(message='用户名是必须的'),
                regexp(RE_MATCH_UKEY,message='用户名只能包含英文,数字,小数点,下划线')])
    password = TextField(validators=[required(message='密码不能为空')])
    email = TextField(validators=[required(message='邮箱是必须的'),email(message='你的邮箱不合法')])
